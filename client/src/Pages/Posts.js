...
const Posts = () => {
  ...
  //console.log(userInfo)
    return (
      ...
      <div style={{display:'flex',flexWrap:'wrap',gap:'35px'}}>
        {postList  && postList.map((item)=>{return(<PostCard comments={item.comments.slice().reverse()} like={item.like} title={item.title} owner={item.owner} _id={item._id} desc={item.desc} image={item.image} />)})}
        </div> 
        <Modal style={{backgroundColor:'cadetblue'}} show={show} onHide={()=>{setShow(false)}}>
  ...
  <Form.Group className="mb-3" controlId="formBasicEmail">
    ...
    type="text" placeholder="Description" style={{outline:'none',padding:'10px'
    ,width:'100%',borderColor:'black',borderWidth:'px',borderRadius:'25px',height:'35px'}} />
  </Form.Group>
  <Form.Group className="mb-3" controlId="formBasicEmail">
